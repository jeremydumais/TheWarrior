...
class SampleGLComponentController : public ::testing::Test {
 public:
  SampleGLComponentController() {
    ...
    map->addTexture({"tex1", "tex1.png", 512, 256, 32, 32});
    map->addTexture({"tex2", "tex2.png", 1024, 512, 32, 32});
...
TEST(GLComponentController_getAlreadyUsedTextureNames,
     DefaultConstructor_ReturnEmptyVector) {
  GLComponentController glComponentController;
  ASSERT_EQ(0, glComponentController.getAlreadyUsedTextureNames().size());
}

TEST(GLComponentController_getAlreadyUsedMonsterZoneNames,
     DefaultConstructor_ReturnEmptyVector) {
  GLComponentController glComponentController;
  ASSERT_EQ(0, glComponentController.getAlreadyUsedMonsterZoneNames().size());
}

TEST(GLComponentController_getAlreadyUsedNPCIds,
     DefaultConstructor_ReturnEmptyVector) {
  GLComponentController glComponentController;
  ASSERT_EQ(0, glComponentController.getAlreadyUsedNPCIds().size());
}

TEST_F(SampleGLComponentController,
       getAlreadyUsedTextureNames_WithTwoTexture_ReturnVectorSizeTwo) {
  ASSERT_EQ(2, glComponentController.getAlreadyUsedTextureNames().size());
}

TEST_F(SampleGLComponentController,
       getAlreadyUsedMonsterZoneNames_WithOneZone_ReturnVectorSizeOne) {
...
TEST_F(SampleGLComponentController,
       getAlreadyUsedNPCIds_WithTwoZones_ReturnVectorSizeTwo) {
...
TEST_F(SampleGLComponentController,
       isTextureUsedInMap_WithUnusedTexture_ReturnFalse) {
  ASSERT_FALSE(glComponentController.isTextureUsedInMap("x"));
}

TEST_F(SampleGLComponentController,
       isTextureUsedInMap_WithUsedTexture_ReturnTrue) {
  ASSERT_TRUE(glComponentController.isTextureUsedInMap("tex1"));
}

TEST_F(SampleGLComponentControllerWithTilesAssigned,
       isShrinkMapImpactAssignedTiles_WithMinusOneOnLeft_ReturnFalse) {
  ASSERT_FALSE(
      glComponentController.isShrinkMapImpactAssignedTiles(-1, 0, 0, 0));
}

TEST_F(SampleGLComponentControllerWithTilesAssigned,
       isShrinkMapImpactAssignedTiles_WithMinusTwoOnLeft_ReturnTrue) {
  ASSERT_TRUE(
      glComponentController.isShrinkMapImpactAssignedTiles(-2, 0, 0, 0));
}

TEST_F(SampleGLComponentControllerWithTilesAssigned,
       isShrinkMapImpactAssignedTiles_WithMinusOneOnTop_ReturnFalse) {
  ASSERT_FALSE(
      glComponentController.isShrinkMapImpactAssignedTiles(0, -1, 0, 0));
}

TEST_F(SampleGLComponentControllerWithTilesAssigned,
       isShrinkMapImpactAssignedTiles_WithMinusTwoOnTop_ReturnTrue) {
  ASSERT_TRUE(
      glComponentController.isShrinkMapImpactAssignedTiles(0, -2, 0, 0));
}

TEST_F(SampleGLComponentControllerWithTilesAssigned,
       isShrinkMapImpactAssignedTiles_WithMinusOneOnRight_ReturnFalse) {
  ASSERT_FALSE(
      glComponentController.isShrinkMapImpactAssignedTiles(0, 0, -1, 0));
}

TEST_F(SampleGLComponentControllerWithTilesAssigned,
       isShrinkMapImpactAssignedTiles_WithMinusTwoOnRight_ReturnTrue) {
  ASSERT_TRUE(
      glComponentController.isShrinkMapImpactAssignedTiles(0, 0, -2, 0));
}

TEST_F(SampleGLComponentControllerWithTilesAssigned,
       isShrinkMapImpactAssignedTiles_WithMinusOneOnBottom_ReturnFalse) {
  ASSERT_FALSE(
      glComponentController.isShrinkMapImpactAssignedTiles(0, 0, 0, -1));
}

TEST_F(SampleGLComponentControllerWithTilesAssigned,
       isShrinkMapImpactAssignedTiles_WithMinusTwoOnBottom_ReturnTrue) {
  ASSERT_TRUE(
      glComponentController.isShrinkMapImpactAssignedTiles(0, 0, 0, -2));
}

TEST_F(SampleGLComponentControllerWithTilesAssigned,
       resizeMap_WithMinusOneOnLeft_ReturnSuccess) {
...
TEST_F(SampleGLComponentController, getMonsterZones_ReturnOneZone) {
  const auto zones = glComponentController.getMonsterZones();
  ASSERT_EQ(1, zones.size());
  ASSERT_EQ("Zone1", zones.at(0).m_name);
}

TEST_F(SampleGLComponentController,
       getMonsterZoneByName_WithEmpty_ReturnEmpty) {
...
TEST_F(SampleGLComponentController, getNPCs_Return2NPCs) {
  const auto npcs = glComponentController.getNPCs();
  ASSERT_EQ(2, npcs.size());
  ASSERT_EQ("NPC001", npcs.at(0).id);
  ASSERT_EQ("NPC002", npcs.at(1).id);
}

TEST_F(SampleGLComponentController, getNPCById_WithEmpty_ReturnEmpty) {
  const auto npcOpt = glComponentController.getNPCById("");
  ASSERT_FALSE(npcOpt.has_value());
}

TEST_F(SampleGLComponentController, getNPCById_WithNPC001_ReturnZone) {
  const auto npcOpt = glComponentController.getNPCById("NPC001");
  ASSERT_TRUE(npcOpt.has_value());
  ASSERT_EQ("NPC001", npcOpt.value().id);
}

TEST_F(SampleGLComponentController,
       applyDenyZone_WithInvalidEventStr_ReturnFalse) {
  ASSERT_FALSE(glComponentController.applyDenyZone("a"));
  ASSERT_EQ("Unrecognized trigger event string a",
            glComponentController.getLastError());
}

TEST_F(
    SampleGLComponentController,
    applyDenyZone_WithMoveUpPressedAndEventIsAlreadyUsedForAnotherAction_ReturnFalse) {
  glComponentController.selectTilesForEditing({1});
  ASSERT_FALSE(glComponentController.applyDenyZone("MoveUpPressed"));
  ASSERT_EQ("Trigger MoveUpPressed is already used for another purpose",
            glComponentController.getLastError());
}

TEST_F(
    SampleGLComponentController,
    applyDenyZone_WithMoveUpPressedAndEventIsAlreadyUsedForAnotherActionOn1Tile_ReturnFalse) {
...
TEST_F(
    SampleGLComponentController,
    applyDenyZone_WithMoveUpPressedAndEventIsAlreadyUsedSameAction_ReturnFalse) {
  glComponentController.selectTilesForEditing({0, 1});
  ASSERT_FALSE(glComponentController.applyDenyZone("MoveUpPressed"));
  ASSERT_EQ("Trigger MoveUpPressed is already used for another purpose",
            glComponentController.getLastError());
...
TEST_F(
    SampleGLComponentController,
    applyDenyZone_WithMoveUpPressedAndEventIsAlreadyUsedSameAction_ReturnTrue) {
  ...
  const auto newTrigger =
      tiles[0]->findConstTrigger(MapTileTriggerEvent::MoveUpPressed);
...
TEST_F(
    SampleGLComponentController,
    applyDenyZone_WithMoveUpPressedAndEventIsAlreadyUsedSameActionOn1Tile_ReturnTrue) {
  glComponentController.selectTilesForEditing({0, 2, 3});
  ASSERT_TRUE(glComponentController.applyDenyZone("MoveUpPressed"));
  const auto tiles = glComponentController.getCurrentMapTiles();
  const auto triggerTile0 =
      tiles[0]->findConstTrigger(MapTileTriggerEvent::MoveUpPressed);
  ...
  const auto newTrigger =
      tiles[1]->findConstTrigger(MapTileTriggerEvent::MoveUpPressed);
  ASSERT_EQ(MapTileTriggerEvent::MoveUpPressed, newTrigger->getEvent());
  ASSERT_EQ(MapTileTriggerCondition::None, newTrigger->getCondition());
  ASSERT_EQ(MapTileTriggerAction::DenyMove, newTrigger->getAction());
  ASSERT_EQ(0, newTrigger->getActionProperties().size());
  const auto triggerTile3 =
      tiles[2]->findConstTrigger(MapTileTriggerEvent::MoveUpPressed);
  ASSERT_EQ(MapTileTriggerEvent::MoveUpPressed, triggerTile3->getEvent());
  ASSERT_EQ(MapTileTriggerCondition::MustBeFacing,
            triggerTile3->getCondition());
  ASSERT_EQ(MapTileTriggerAction::DenyMove, triggerTile3->getAction());
  ASSERT_EQ(1, triggerTile3->getActionProperties().size());
}

TEST_F(SampleGLComponentController,
       clearDenyZones_WithNoTriggersTile_ReturnSuccess) {
...
TEST_F(SampleGLComponentController,
       clearDenyZones_WithOneTriggerTile_ReturnSuccess) {
  glComponentController.selectTilesForEditing({0});
  glComponentController.clearDenyZones();
  const auto tiles = glComponentController.getCurrentMapTiles();
  ASSERT_EQ(1, tiles.size());
...
TEST_F(SampleGLComponentController,
       clearDenyZones_WithOneTriggerNotDenyMove_ReturnSuccess) {
  glComponentController.selectTilesForEditing({1});
  glComponentController.clearDenyZones();
  const auto tiles = glComponentController.getCurrentMapTiles();
  ASSERT_EQ(1, tiles.size());
...
TEST_F(SampleGLComponentController,
       clearDenyZones_WithTwoTriggersDenyMoveAndOneNot_ReturnSuccess) {
...
TEST_F(SampleGLComponentController,
       setUseOnlyOneMonsterZone_WithFalse_ReturnTrue) {
...
TEST_F(SampleGLComponentControllerWithTilesAssigned,
       setUseOnlyOneMonsterZone_WithTrue_ReturnFalse) {
...
TEST_F(SampleGLComponentControllerWithTilesAssigned,
       isUseOnlyOneMonsterZone_ReturnFalse) {
  ASSERT_FALSE(glComponentController.isUseOnlyOneMonsterZone());
}
